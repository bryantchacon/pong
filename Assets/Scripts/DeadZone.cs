...
public class DeadZone : MonoBehaviour {
    ...
    /*
    private void OnCollisionEnter2D(Collision2D collision) //Detecta cuando algo(representado por la variable local collision) CHOCA con el collider del gameobject cuando este NO ES un trigger
    {
        Debug.Log("Colision");
    }
    */

    private void OnTriggerEnter2D(Collider2D ball) //Detecta cuando algo(representado por la variable local ball) ATRAVIEZA el collider del game object cuando este ES un trigger
    {
    ...
    //Esto es parecido en la variable label de la funcion UpdateScoreLabel(), se declara con la clase Text como tipo de dato, y para acceder a la variable text de label(que ahora tiene por Text) se hace con .text en label.text

    void CheckScore() //Para que esta funcion funcione; el script Scene Changer(dentro de un componente vacio) debe agregarse como parametro desde el editor, dentro de la variable que le corresponde en este script
    {
